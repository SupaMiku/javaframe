...
void displayMyTransactions() {
    ...
    if (userType.equals("USER")) {
        // ✅ USER — search only within their own transactions
        int a_id = config.session.getInstance().getA_id();
        String sql;
        if (column.equals("trans_id")) {
            sql = "SELECT trans_id, title, borrow_date, return_date, status "
                + "FROM tbl_transaction WHERE a_id = ? "
                + "AND CAST(trans_id AS TEXT) LIKE ?";
        } else {
            sql = "SELECT trans_id, title, borrow_date, return_date, status "
                + "FROM tbl_transaction WHERE a_id = ? "
                + "AND " + column + " LIKE ?";
        }
        con.displayData(sql, usertable, a_id, "%" + keyword + "%");
    } else {
    ...
    private void initComponents() {
        ...
        navbar.add(bookspanel, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 180, 230, 50));
    ...
    // ✅ Use the public method instead of accessing private fields directly
    String userType = config.session.getInstance().getType();
    if (userType.equals("USER")) {
        form.prefillUser(
            config.session.getInstance().getA_id(),
            config.session.getInstance().getUsername()
        );
    }
    ...
    try {
        ...
        String a_id        = safeGet(row, 1);
        String user_name   = safeGet(row, 2);
        String bok_id      = safeGet(row, 3);
        String title       = safeGet(row, 4);
