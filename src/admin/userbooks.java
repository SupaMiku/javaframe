...
if (userType.equals("USER")) {
    ...
    searchColumn.setBounds(540, 10, 120, 40);
    ...
    jPanel1.add(searchColumn, new org.netbeans.lib.awtextra.AbsoluteConstraints(310, 10, 120, 40));

    // Reposition searchfilter
    searchfilter.setBounds(440, 10, 160, 40);
    ...
    searchfilter.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
        ...
        public void removeUpdate(javax.swing.event.DocumentEvent e) {
            if (searchfilter.getText().trim().isEmpty()) displayBooks();
        }
    ...
    void displayBooks() {
    conf con = new conf();
    con.displayData("SELECT * FROM tbl_books", usertable);
}
    ...
    String sql;
    if (column.equals("bok_id")) {
        sql = "SELECT * FROM tbl_books WHERE CAST(bok_id AS TEXT) LIKE ?";
    } else {
        sql = "SELECT * FROM tbl_books WHERE " + column + " LIKE ?";
    }
    con.displayData(sql, usertable, "%" + keyword + "%");
}
    ...
    private void addActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addActionPerformed
       addeditbooks form = new addeditbooks(); // ✅ correct class name
    form.setAddMode();
    ...
    try {
        int    bok_id   = Integer.parseInt(safeGet(row, 0));
        String title    = safeGet(row, 1);
        String author   = safeGet(row, 2);
        String genre    = safeGet(row, 3);
        String quantity = safeGet(row, 4);
        String status   = safeGet(row, 5);

        addeditbooks form = new addeditbooks(); // ✅ correct class name
        form.setEditMode(bok_id, title, author, genre, quantity, status);
        form.setVisible(true);
        form.addWindowListener(new java.awt.event.WindowAdapter() {
            @Override
            public void windowClosed(java.awt.event.WindowEvent e) {
    ...
    if (choice == JOptionPane.YES_OPTION) {
        conf con = new conf();
        boolean ok = con.executeUpdate("DELETE FROM tbl_books WHERE bok_id = ?", bok_id);
        if (ok) {
            JOptionPane.showMessageDialog(this, "Book deleted successfully.", "Success",
                JOptionPane.INFORMATION_MESSAGE);
            displayBooks();
        } else {
            JOptionPane.showMessageDialog(this, "Failed to delete book.", "Error",
    ...
    private void transactionpanelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_transactionpanelMouseClicked
       usertransaction transc = new usertransaction();
